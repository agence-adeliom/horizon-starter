<div x-data x-show="open" x-transition:enter="transition-opacity ease-out duration-300"
    ...
    <div x-show="open" x-transition:enter="transition-transform ease-out duration-300"
        ...
        role="dialog"
        aria-modal="true"
        @isset($header) aria-labelledby="drawer-{{ $attributes['id'] ?? 'default' }}-title" @endisset
        @class([
        ...
        <!-- Header Slot -->
        @isset($header)
            <div id="drawer-{{ $attributes['id'] ?? 'default' }}-title" @class(['px-4 py-6 lg:px-6', $header->attributes['class']])>
                {{ $header }}
            </div>
        @endisset
