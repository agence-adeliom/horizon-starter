...
document.addEventListener('alpine:init', () => {
    window.Alpine.data('initArgumentsSlider', () => {
        return {
            ...
            init() {
                ...
                const swiperParams: SwiperOptions = {
                    ...
                    pagination: {
                        ...
                        type: 'bullets',
                        bulletClass: 'swiper-bullet flex w-full gap-3 cursor-pointer overflow-hidden opacity-60 transition-opacity',
                    ...
                    on: {
                        paginationUpdate: (swiper, paginationEl) => {
                            ...
                            const bulletProgress: NodeListOf<HTMLSpanElement> = this.$refs.pagination.querySelectorAll(
                                `.${classes.progress}`,
                            );
                            if (bulletProgress.length) {
                                bulletProgress.forEach((item: HTMLSpanElement) => {
                                    const inner: HTMLSpanElement = item.querySelector(`.${classes.progressInner}`);
            ...
            handleBulletClick() {
                this.$el.querySelectorAll(`.${classes.progress}`).forEach((bullet: HTMLSpanElement) => {
                    const inner: HTMLSpanElement = bullet.querySelector(`.${classes.progressInner}`);
                    inner.style.animationPlayState = 'running';
                });
            },
            togglePause() {
                const activeProgress: HTMLSpanElement = this.$root.querySelector(`.${classes.bullet}.is-active`);
                const activeInner: HTMLSpanElement = activeProgress && activeProgress.querySelector(`.${classes.progressInner}`);

                if (activeInner) {
                    activeInner && (activeInner.style.animationPlayState = this.swiper.autoplay.paused ? 'running' : 'paused');
                    this.swiper.autoplay.paused ? this.swiper.autoplay.resume() : this.swiper.autoplay.pause();
